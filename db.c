...
void initialize_internal_node(void* node) {
  ...
  *internal_node_num_keys(node) = 0;
  /*
  Necessary because root page num is 0; this can avoid
  making the new internal node a parent of the root.
  */
...
void internal_node_split_and_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
  ...
  */
  ...
  if (splitting_root) {
    ...
    to the new root's left child, new_page_num will already point to the
  ...
  uint32_t* old_num_key = *internal_node_num_keys(old_node);

  uint32_t cur_page_num = *internal_node_right_child(old_node);
  void* cur = get_page(table->pager, cur_page_num);
  /*
  First put right child into new node and set right child of old_node to invalid page number.
  */
